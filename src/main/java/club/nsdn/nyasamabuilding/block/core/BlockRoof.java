...
/**
 * Created by drzzm32 on 2019.9.3.
 */
public class BlockRoof extends net.minecraft.block.BlockFence {
    ...
    public BlockRoof(IBlockState state, String name, String id) {
    ...
    public boolean canConnectTo(IBlockAccess world, @Nonnull BlockPos pos, @Nonnull EnumFacing facing) {
        ...
        boolean flag = shape == BlockFaceShape.MIDDLE_POLE && (block instanceof BlockRoof || block instanceof BlockFenceGate);
    ...
    public void addCollisionBoxToList(
        ...
        if (up) {
            addCollisionBoxToList(pos, aabb, list, Block.FULL_BLOCK_AABB);
        } else {
    ...
    public AxisAlignedBB getBoundingBox(@Nonnull IBlockState state, @Nonnull IBlockAccess world, @Nonnull BlockPos pos) {
        ...
        boolean down = state.getValue(DOWN);
        boolean up = state.getValue(UP);
        return up ? Block.FULL_BLOCK_AABB : BOUNDING_BOXES[getBoundingBoxIdx(state)].contract(0, down ? 0 : -0.5, 0);
    }
    ...
    public IBlockState getActualState(IBlockState state, IBlockAccess world, BlockPos pos) {
        return super.getActualState(state, world, pos)
                .withProperty(UP, !(world.getBlockState(pos.up()).getBlock() instanceof net.minecraft.block.BlockFence))
                .withProperty(DOWN,
